<template>
  <q-page class="container q-layout-padding">
    ...
    <q-table
      ...
      :rows="state.list"
      :columns="medicalAreaTableColumns"
      :filter="state.filter"
    ...
    >
      <template #top-right>
        <action-header
          label-new-entity="Nova área médica"
        ...
        <q-form @submit="save">
          <q-card-section class="q-py-none q-pt-sm">
            <h6 class="text-h6 q-my-none">
              {{ state.form.id ? 'Editar' : 'Criar' }} área médica
            </h6>
          </q-card-section>
...
import { useMedicalArea } from './useMedicalArea'
import { medicalAreaTableColumns } from './medicalArea.const'
