...
interface IState {
  ...
  actionsData: IMedicalArea[]
}

export function useMedicalArea() {
  ...
  async function fetchList() {
    await requester.dispatch({
      callback: async () => {
        state.value.list = await MedicalAreaService.getAll()
      },
  ...
  async function save() {
    ...
    await requester.dispatch({
      callback: async () => {
        if (id)
          await MedicalAreaService.save(
            id,
            state.value.form.name,
            state.value.form.imageFile,
          )
        else
          await MedicalAreaService.create(
            state.value.form.name,
            state.value.form.imageFile!,
          )
      },
    ...
    await requester.dispatch({
      callback: async () => {
        ...
        if (actionType == ActionDialogOptions.delete)
          await MedicalAreaService.deleteItem(ids)
        if (actionType == ActionDialogOptions.disable)
          await MedicalAreaService.disable(ids)
      },
      successCallback: async () => {
  ...
  function openEditDialog(item?: IMedicalArea) {
    if (item)
      state.value.form = {
        ...item,
        imageFile: null,
